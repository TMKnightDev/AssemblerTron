...
void sw_immd_assm(void) {
	...
	*/
	...
	if (PARAM2.type != REGISTER) { // was PARAM3, fixed
		state = MISSING_REG;
		return;
	}
	// The offset/immediate should be an immediate
	if (PARAM3.type != IMMEDIATE) { // was PARAM2, fixed
		state = INVALID_PARAM;	
		return;
	}
...
// Rs should be 31 or less
	...
	if (PARAM3.value > 0xFFFF) {  // immediate should be 16 bits, was PARAM2 fixed
		state = INVALID_IMMED;
		return;
	}
	...
	*/
	...
	// set Rs
	setBits_num(25, PARAM2.value, 5); // was PARAM3, fixed
	// set immediate/offset
	setBits_num(15, PARAM3.value, 16); //offset was PARAM2, fixed
